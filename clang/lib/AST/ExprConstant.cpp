...
static bool IsGlobalLValue(APValue::LValueBase B) {
  ...
  // The APValue generated from a
  // __builtin_source_location/__builtin_source_location_at will be emitted as a
  // literal.
...
public:
  ...
  bool VisitSourceLocExpr(const SourceLocExpr *E) {
    ...
    std::optional<SourceLocation> PrecomputedLoc = std::nullopt;
    if (const Expr *DistanceArgExpr = E->getCallStackDistance()) {
      APSInt DistanceValue;
      if (::EvaluateInteger(DistanceArgExpr, DistanceValue, Info) &&
          DistanceValue.getBitWidth() <= 64) {
        // The distance value is based on the current call and doesn't do the
        // default expression shenanigans.
        uint64_t CallStackDistance = DistanceValue.getZExtValue();
        if (CallStackDistance < 1) {
          SourceLocation TargetLoc = E->getBeginLoc();
          if (TargetLoc.isValid())
            PrecomputedLoc = TargetLoc;
        } else {
          ...
          auto TargetFrameAndDepth =
              Info.getCallFrameAndDepth(TargetFrameIndex);
          SourceLocation TargetLoc =
              TargetFrameAndDepth.first->CallRange.getBegin();
          if (TargetLoc.isValid())
            PrecomputedLoc = TargetLoc;
        }
      }
...
bool PointerExprEvaluator::VisitBuiltinCallExpr(const CallExpr *E,
  ...
  case Builtin::BI__builtin_std_embed: {
    ...
    // The dependency-pattern machinery is set up by the frontend when embed
    // support is enabled; if it is somehow absent, diagnose instead of
    // dereferencing a null pointer below (which would crash the compiler).
    if (!Info.Ctx.InputDependencyPatterns) {
      Info.FFDiag(ResourceNamePtrArg->getBeginLoc(), diag::err_cannot_open_file)
          << ResourceName
          << "the resource-dependency machinery is unavailable in this "
             "compilation";
      return false;
    }
    ...
    DataSize = std::min<size_t>(FullDataSize - DataOffset,
    ...
    // Read exactly enough of the file to cover the requested
    // [DataOffset, DataOffset + DataSize) window. The buffer size hint is the
    // number of bytes to read from the start, so it must account for the
    // offset - passing the bare limit here would read too little and make the
    // DataOffset slice below read out of bounds (and disagree with the size we
    // already reported). DataOffset + DataSize is guaranteed <= FullDataSize.
    const int64_t BytesToRead = static_cast<int64_t>(DataOffset + DataSize);
    llvm::ErrorOr<std::unique_ptr<llvm::MemoryBuffer>> MaybeBinaryData =
        FM.getBufferForFile(*ResourceFile, true, false, BytesToRead, false);
    ...
    assert(BinaryData->getBufferSize() >= DataOffset + DataSize &&
           "the resource buffer does not cover the requested "
           "[offset, offset + size) window");
